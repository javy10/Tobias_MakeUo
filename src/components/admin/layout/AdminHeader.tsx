...
export function AdminHeader({ 
  ...
  return (
        <motion.header
          className="sticky top-0 z-30 flex h-16 items-center gap-2 sm:gap-4 border-b bg-background/95 backdrop-blur-sm px-3 sm:px-6"
      ...
      <motion.div 
        className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1 ml-1 sm:ml-0"
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.5, delay: 0.1 }}
      >
            <h1 className="text-sm sm:text-base md:text-lg font-semibold truncate text-foreground">{pageName}</h1>
      </motion.div>

      <motion.div 
        className="ml-auto flex items-center gap-1.5 sm:gap-2 md:gap-3"
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
      ...
      >
        ...
        >
          <Button 
            ...
            size="icon" 
            onClick={toggleTheme}
                className="hover:bg-accent text-foreground h-9 w-9 sm:h-10 sm:w-10"
        ...
        >
          <DropdownMenu>
              <DropdownMenuTrigger asChild>
                      <Button variant="ghost" className="flex items-center gap-1 sm:gap-1.5 h-9 sm:h-10 px-2 sm:px-2.5 text-foreground hover:bg-accent">
        ...
        >
          <DropdownMenu>
              <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="relative h-9 w-9 sm:h-10 sm:w-10 text-foreground hover:bg-accent">
                      <Bell className="h-4 w-4" />
                      {unseenPendingTestimonials.length > 0 && (
                         <motion.span 
                           className="absolute top-0 right-0 inline-flex items-center justify-center px-1 py-0.5 sm:px-1.5 sm:py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full min-w-[16px] sm:min-w-[18px]"
                           initial={{ scale: 0 }}
                           animate={{ scale: 1 }}
        ...
        >
          <DropdownMenu>
              <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-9 w-9 sm:h-10 sm:w-10 text-foreground hover:bg-accent">
